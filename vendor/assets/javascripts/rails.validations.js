...
(function() {
  var $, validateElement, validateForm, validateRadioButton, validatorsFor,
  ...
  validateForm = function(form, validators) {
    var groupedRadioButtons, radioButtons, valid;
    ...
    if (radioButtons) {
      groupedRadioButtons = _.groupBy(radioButtons, function(button) {
        return $(button).attr("name");
      });
      _.each(groupedRadioButtons, function(buttonGroup, name) {
  ...
  window.ClientSideValidations.enablers = {
    form: function(form) {
      var $form, binding, event, radioButtonGroups, _ref;
      $form = $(form);
      ...
      radioButtonGroups = _.groupBy($form.find('[data-validate="true"]input[type=radio]'), function(button) {
        return $(button).attr("name");
      });
      return _.each(radioButtonGroups, function(radioButtonGroup, name) {
        var commonParentSelector;
        commonParentSelector = "#" + $(radioButtonGroup).first().parents(":not(label)").first().prop("id");
        if (commonParentSelector.length > 0) {
          $form.on('element:validate:after.ClientSideValidations', commonParentSelector, function(eventData) {
            return ClientSideValidations.callbacks.element.after($(this), eventData);
          });
          $form.on('element:validate:before.ClientSideValidations', commonParentSelector, function(eventData) {
            return ClientSideValidations.callbacks.element.before($(this), eventData);
          });
          $form.on('element:validate:fail.ClientSideValidations', commonParentSelector, function(eventData, message) {
            var element;
            element = $(this);
